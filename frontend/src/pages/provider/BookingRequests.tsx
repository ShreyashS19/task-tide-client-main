...
export default function BookingRequests() {
  ...
  const setStatus = async (bookingId: number, status: BookingStatus) => {
    try {
      ...
      toast({ title: "Error", description: e?.message || "Failed to update booking", variant: "destructive" });
    }
  };
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-4">
              {bookings.map((b) => (
                <div key={b.bookingId} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border rounded-lg gap-4">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-foreground">{b.serviceType}</h3>
                      {getStatusBadge(b.status)}
                    </div>
                    <p className="text-sm text-muted-foreground">{b.bookingDate} at {b.bookingTime?.slice(0,5)}</p>
                  </div>
                  <div className="flex gap-2">
                    {b.status === "PENDING" && (
                      <>
                        <Button size="sm" onClick={() => setStatus(b.bookingId, "ACCEPTED")}><CheckCircle className="h-4 w-4 mr-2" />Accept</Button>
                        <Button size="sm" variant="destructive" onClick={() => setStatus(b.bookingId, "REJECTED")}><XCircle className="h-4 w-4 mr-2" />Reject</Button>
                      </>
                    )}
                    {b.status === "ACCEPTED" && (
                      <Button size="sm" variant="outline" onClick={() => setStatus(b.bookingId, "COMPLETED")}>Mark Completed</Button>
                    )}
                  </div>
                </div>
              ))}
